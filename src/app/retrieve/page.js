...
export default function RetrieveCarPage() {
  ...
  const handleJoinQueue = async () => {
    ...
    console.log('1. Selected lift:', selectedLift);
    console.log('2. User ID:', userId);
    console.log('3. Current time:', new Date().toISOString());
  
    // Check if already in queue
    console.log('4. Checking if user already in queue...');
    ...
    if (checkError) {
      console.error('5. Error checking existing:', checkError);
    }
  
    if (existing) {
      console.log('5. User already in queue:', existing);
      alert('You are already in the queue!');
      return;
    }
    console.log('5. User not in queue, proceeding...');
  
    // Join queue
    const now = new Date();
    console.log('6. Inserting user with data:', {
      user_id: userId,
      lift: selectedLift,
      status: 'waiting',
    ...
    if (error) {
      console.error('7. ERROR inserting user:', error);
      alert('Failed to join queue: ' + error.message);
    } else {
      console.log('7. SUCCESS! User inserted:', data);
      console.log('8. Setting showPositionVerifier to true');
      setJustJoinedLift(selectedLift);
      setShowPositionVerifier(true);
    }
    console.log('========== END JOIN QUEUE ==========');
  };
  ...
  const handleVerifyPosition = async (position) => {
    ...
    try {
      ...
      console.log('Current queue length:', digitalQueueLength);
      
      // CASE: User reports MORE people than digital queue
      if (position > digitalQueueLength) {
        ...
        for (let i = 0; i < missingPeople; i++) {
          ...
          const phantomTime = new Date(Date.now() - (phantomPosition * 60000));
          const phantomId = `phantom-${Date.now()}-${phantomPosition}-${Math.random().toString(36).substr(2, 8)}`;
          ...
          if (error) {
            console.error('Error adding phantom:', error);
            alert('Failed to add phantom entries: ' + error.message);
            return;
          }
        }
      }
      ...
      // Place user at reported position
      let targetTime;
      ...
      alert(`✅ Queue updated to ${updatedQueue.length} people. You are #${newPosition}.`);
      await loadQueueData();
